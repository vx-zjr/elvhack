...
- 2026-06-13: Added anonymous comments and likes for both article posts and Lab project cards. Introduced `GET /api/interactions`, generalized `POST /api/comments` and `POST /api/reactions` to `target_type` / `target_slug`, added browser-local anonymous UUID deduplication for likes, and added Supabase migration `20260613010000_interaction_targets.sql` for existing databases. Verified `npm run typecheck`, `npm run lint`, `npm run test`, and `npm run build` locally with the bundled Node/npm shim. Environment still lacks `.env.local`, Supabase CLI, `npx`, and `CLOUDFLARE_API_TOKEN`, so real Supabase migration and Cloudflare Pages secret writes require credentials outside this shell.
- 2026-06-13: Pushed anonymous interaction commit `00e38c4` to GitHub `main` and verified Cloudflare Pages production at `https://elvhack.com/` serves the new build assets `index-DWpbGlio.js` and `index-BKBJwxW8.css`. Verified `GET /api/interactions` reaches the deployed Pages Function and returns structured `UPSTREAM_ERROR` / `Supabase service configuration is missing.` until production Supabase env vars are configured.
- 2026-06-13: Replaced the homepage-only particle field with a global interactive smoke canvas rendered from `SiteLayout`, added dark/light theme switching with localStorage persistence, and added Tailwind `light:` variant styling plus global light-theme fallbacks. Removed the homepage-specific `ImmersiveField` from the first viewport so the smoke field is shared across all routes. Verified local desktop and mobile browser layouts: smoke canvas covers the viewport, theme toggle switches DOM/theme colors, and there is no horizontal overflow. Re-ran `npm run typecheck`, `npm run lint`, `npm run test`, and `npm run build` successfully.
- 2026-06-13: Pushed global smoke/theme commit `efc7bfb` to GitHub `feature/edge-cms-rebuild` and `main`. Verified Cloudflare Pages production at `https://elvhack.com/` serves the new smoke/theme build assets `index-DaaOrep0.js` and `index-iAOrEdZJ.css`.
...
- Apply Supabase migrations `20260613000000_initial_elvhack_cms.sql` and `20260613010000_interaction_targets.sql` to the target Supabase project.
