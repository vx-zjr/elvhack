...
async function getLikes(env: Env, request: Request) {
...
async function postLike(env: Env, request: Request) {
	if (!env.DB) return error("D1 binding DB is not configured.", 500);
...
async function handleLikesRequest(env: Env, request: Request) {
	if (request.method === "GET") return getLikes(env, request);
	if (request.method === "POST") return postLike(env, request);
...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		if (url.pathname === "/api/likes") {
			return handleLikesRequest(env, request);
		}
